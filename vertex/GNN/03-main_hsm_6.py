...
def run_gnn(num_folds, current_fold):
    ...
    parser.add_argument('--dataroot', type=str, default='/home/alex/hsm/GNN/BrainGNN_Pytorch/data/APOE2/full_connect/sex_vertex_32', help='root directory of the dataset')
    ...
    parser.add_argument('--ratio', type=float, default=0.25, help='pooling ratio')
    parser.add_argument('--indim', type=int, default=32, help='feature dim')
    parser.add_argument('--nroi', type=int, default=32, help='num of ROIs')
...
#np.save('kfold_results.npy', kfold_results)
    return len(te_index), test_accuracy



if __name__=='__main__':
    num_folds = 5
    results = np.zeros([2,num_folds])
    results[0,0], results[1,0] = run_gnn(num_folds,0)
    results[0,1], results[1,1] = run_gnn(num_folds,1)
    results[0,2], results[1,2] = run_gnn(num_folds,2)
    results[0,3], results[1,3] = run_gnn(num_folds,3)
    results[0,4], results[1,4] = run_gnn(num_folds,4)
    ...
    np.save('test_acc_results_sex_32_5fold_6.npy', results)
